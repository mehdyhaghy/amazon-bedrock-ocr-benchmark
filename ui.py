...
def create_input_panel():
    ...
    return panel, sample_dropdown, input_image, refresh_samples
...
def create_common_options_panel():
    ...
    with gr.Column() as panel:
        ...
        gr.Markdown("**Desired Output JSON Schema**")
    ...
    return panel, document_type, output_schema

def create_bedrock_options_panel():
    """Create the Bedrock options panel"""
    ...
    return panel, bedrock_model

def create_bda_options_panel():
    """Create the BDA options panel"""
    with gr.Column(visible=False) as panel:
        bda_s3_bucket = gr.Textbox(
            label="S3 Bucket for BDA Processing",
            value="my-bda-demo-bucket",
            placeholder="Enter your S3 bucket name (required)",
            info="S3 bucket where images will be uploaded for processing"
        )
    ...
    return panel, bda_s3_bucket, use_bda_blueprint
